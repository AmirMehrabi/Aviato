...
class VirtualMachineController extends Controller
{
    ...
    public function moveNode(Request $request): RedirectResponse
    {
        ...
        MoveVirtualMachinesToNodeJob::dispatch(
            array_map('intval', $data['vm_ids']),
            $targetNode,
            $mode,
            $online,
            $request->user('admin')?->getAuthIdentifier(),
        )->onQueue(MoveVirtualMachinesToNodeJob::QUEUE);

        return back()
            ->with('status', count($data['vm_ids']).' VM node move task(s) queued. You will receive an admin notification when processing finishes.');
    }
    ...
    private function formData(VirtualMachine $vm): array
    {
        ...
        return [
            ...
            'projects' => Project::query()->with('owner')->orderBy('name')->get(),
            'servers' => $servers,
            'nodeMoveOptions' => $this->nodeMoveOptions($servers),
            'bundles' => VmBundle::query()->where('is_active', true)->orderBy('sort_order')->orderBy('monthly_price')->get(),
    ...
    private function nodeMoveOptions(iterable $servers): array
    {
        return collect($servers)
            ->flatMap(function (ProxmoxServer $server) {
