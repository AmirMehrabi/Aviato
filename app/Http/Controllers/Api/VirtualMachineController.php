...
class VirtualMachineController extends Controller
{
    ...
    {
        $customer = $this->customer($request);
        if (! $this->projects->canManageVms($project, $customer)) {
            return $this->error('You do not have VM management access to this project.', 'project_forbidden', 403);
        }
        ...
        if (! $image || ! $location) {
            return $this->error('The selected image or location is not available.', 'invalid_creation_option', 422);
        }
        if ($data['vm_bundle_id'] ?? false ? ! $bundle : false) {
            return $this->error('The selected bundle is not active.', 'invalid_creation_option', 422);
        }
        if (! $this->imageAvailable($image, $location)) {
            return $this->error('The selected image is not available in this location.', 'image_location_incompatible', 422);
        }
        if (! $this->bundleAvailable($image, $location, (int) ($data['vm_bundle_id'] ?? 0))) {
            return $this->error('The selected bundle is not available for this image and location.', 'bundle_incompatible', 422);
        }
        $quota = $this->quota->snapshot($project->owner);
        $wallet = $this->wallets->walletFor($project->owner);
        if (! $quota['can_create']) {
        ...
        if ($bundle && $wallet->balance < $this->minimumBalance($bundle, $location)) {
            return $this->error('The project wallet does not meet the minimum creation balance.', 'insufficient_balance', 422);
        }
        if ($location->isProxmox() && $this->availableIpCount($image) < 1) {
            return $this->error('No IP capacity is available for the selected image.', 'ip_capacity_unavailable', 422);
        }
        ...
        try {
            ...
            $price = $bundle ? $this->effectivePrice($bundle, $location) : 0;
            $charge = $bundle ? AppSetting::vmCreationChargeAmount($price) : 0;
            if ($charge > 0) {
                $this->wallets->charge($project->owner, $charge, 'VM creation fee '.$result['vm']->display_name, $result['vm'], ['category' => 'vm_creation_fee', 'monthly_price' => $price, 'provider' => $location->provider]);
            }
            ...
            return $this->error('VM creation failed: '.$e->getMessage(), 'creation_failed', 422);
        }
    }
    ...
    public function destroy(Request $request, Project $project, VirtualMachine $virtualMachine): VirtualMachineResource|JsonResponse
    {
        ...
        $vm = $this->visibleVm($request, $project, $virtualMachine, true);
        $request->validate(['confirmation' => ['required', 'string', Rule::in([$vm->display_name])]]);
    ...
    private function visibleVm(Request $request, Project $project, VirtualMachine $vm, bool $manage = false): VirtualMachine
    {
        $customer = $this->customer($request);
        if ($vm->project_id !== $project->id || ! ($manage ? $this->projects->canManageVms($project, $customer) : $this->projects->canViewVms($project, $customer))) {
            abort(404);
        }
    ...
    {
        return InfrastructureLocation::query()->with('hetznerAccount')->where('is_active', true)->where('maintenance_mode', false)->when($id, fn ($q) => $q->whereKey($id))->when(! $id && $image->infrastructure_location_id, fn ($q) => $q->whereKey($image->infrastructure_location_id))->when(! $id && ! $image->infrastructure_location_id, fn ($q) => $q->where('proxmox_server_id', $image->proxmox_server_id))->first();
    }

    private function imageAvailable(CloudImage $image, InfrastructureLocation $location): bool
    {
        return $location->isProxmox() ? $image->isProxmox() && ((int) $image->infrastructure_location_id === (int) $location->id || (int) $image->proxmox_server_id === (int) $location->proxmox_server_id) : $image->isHetzner() && (int) data_get($image->provider_metadata, 'hetzner_account_id') === (int) $location->hetzner_account_id;
    }

    private function bundleAvailable(CloudImage $image, InfrastructureLocation $location, int $id): bool
    {
        return $id <= 0 ? $location->isProxmox() : ($location->isProxmox() ? $image->allowedBundles->contains('id', $id) : VmBundleLocationMapping::query()->where('infrastructure_location_id', $location->id)->where('vm_bundle_id', $id)->where('is_active', true)->whereNotNull('hetzner_server_type_id')->exists());
    }

    private function availableIpCount(CloudImage $image): int
    {
        if (! $image->isProxmox()) {
            return 999999;
        } $nodes = $image->nodeMappings->where('is_enabled', true)->pluck('node')->filter()->unique();
    ...
    private function effectivePrice(VmBundle $bundle, InfrastructureLocation $location): int
    {
        ...
        } $mapping = VmBundleLocationMapping::query()->with('hetznerServerType')->where('vm_bundle_id', $bundle->id)->where('infrastructure_location_id', $location->id)->where('is_active', true)->first();

        return (int) AppSetting::convertHetznerUsdToIrr((float) ($mapping?->monthly_price_usd ?? 0));
    }

    private function minimumBalance(VmBundle $bundle, InfrastructureLocation $location): int
    {
        $price = $this->effectivePrice($bundle, $location);
    ...
    {
        $errors = $validator instanceof ValidationException ? $validator->errors() : $validator->errors();

        return response()->json(['error' => ['code' => 'validation_error', 'message' => 'One or more fields are invalid.', 'fields' => $errors], 'meta' => ['request_id' => request()->attributes->get('api_request_id')]], 422);
    }
}
