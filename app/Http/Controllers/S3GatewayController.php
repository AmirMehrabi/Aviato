...
class S3GatewayController extends Controller
{
    ...
    private function dispatch(Request $request, ?StorageBucket $bucket, string $key): Response|StreamedResponse
    {
        ...
        if ($key !== '' && $request->has('uploads')) {
            return $this->initiateMultipart($request, $bucket, $key);
        }
    ...
    private function bucket(?string $name, StorageAccessKey $accessKey): ?StorageBucket
    {
        ...
        if (! $bucket) {
            throw new S3Exception('NoSuchBucket', 'The specified bucket does not exist.', 404, $name);
        }
        if ((int) $bucket->project_id !== (int) $accessKey->project_id) {
            throw new S3Exception('AccessDenied', 'You do not have access to this bucket.', 403, $name);
        }
    ...
    private function listBuckets(Request $request): Response
    {
        ...
        foreach ($buckets as $bucket) {
            $xml .= '<Bucket><Name>'.e($bucket->name).'</Name><CreationDate>'.e($bucket->created_at->toIso8601String()).'</CreationDate></Bucket>';
        }

        return $this->xml($xml.'</Buckets></ListAllMyBucketsResult>');
    }

    private function bucketCreated(StorageBucket $bucket): Response
    {
        return response('', 200);
    }
    ...
    private function listObjects(Request $request, StorageBucket $bucket): Response
    {
        ...
        if ($request->filled('continuation-token')) {
            $startAfter = base64_decode(strtr((string) $request->query('continuation-token'), '-_', '+/')) ?: '';
        }
        ...
        foreach ($commonPrefixes as $commonPrefix => $_) {
            $xml .= '<CommonPrefixes><Prefix>'.e($commonPrefix).'</Prefix></CommonPrefixes>';
        }
        foreach ($contents as $object) {
            $xml .= '<Contents><Key>'.e($object->object_key).'</Key><LastModified>'.e($object->updated_at->toIso8601String()).'</LastModified><ETag>&quot;'.e($object->etag).'&quot;</ETag><Size>'.$object->size_bytes.'</Size><StorageClass>STANDARD</StorageClass></Contents>';
        }
    ...
    private function uploadPart(Request $request, StorageBucket $bucket, string $key): Response
    {
        ...
        if ($partNumber === false || $partNumber < 1 || $partNumber > 10000) {
            throw new S3Exception('InvalidPart', 'The part number must be between 1 and 10000.', 400);
        }
    ...
    private function listParts(Request $request, StorageBucket $bucket, string $key): Response
    {
        ...
        foreach ($parts as $part) {
            $xml .= '<Part><PartNumber>'.$part->part_number.'</PartNumber><LastModified>'.e($part->updated_at->toIso8601String()).'</LastModified><ETag>&quot;'.e($part->etag).'&quot;</ETag><Size>'.$part->size_bytes.'</Size></Part>';
        }
    ...
    private function completeMultipart(Request $request, StorageBucket $bucket, string $key): Response
    {
        ...
        if ($requested === false) {
            throw new S3Exception('MalformedXML', 'The multipart completion XML is invalid.', 400);
        }
        ...
        if ($partNumbers->isEmpty() || $parts->count() !== $partNumbers->unique()->count()) {
            throw new S3Exception('InvalidPart', 'One or more requested parts are missing.', 400);
        }
    ...
    private function multipart(Request $request, StorageBucket $bucket, string $key): StorageMultipartUpload
    {
        ...
        if (! $upload || $upload->expires_at->isPast()) {
            throw new S3Exception('NoSuchUpload', 'The specified multipart upload does not exist.', 404);
        }
    ...
    private function getObject(Request $request, StorageBucket $bucket, string $key): StreamedResponse
    {
        ...
        $etag = '"'.$object->etag.'"';
        if ($request->header('If-None-Match') === $etag || $request->header('If-None-Match') === $object->etag) {
            return response('', 304)->header('ETag', $etag);
        }
        if ($request->hasHeader('If-Match') && trim((string) $request->header('If-Match'), '"') !== $object->etag) {
            throw new S3Exception('PreconditionFailed', 'The object ETag does not match the If-Match condition.', 412);
        }
        $start = 0;
        ...
        if ($range !== null) {
            if (! preg_match('/^bytes=(\d*)-(\d*)$/', trim($range), $matches) || ($matches[1] === '' && $matches[2] === '')) {
                throw new S3Exception('InvalidRange', 'Only a single byte range is supported.', 416);
            }
            if ($matches[1] === '') {
                $length = (int) $matches[2];
                $start = max(0, $object->size_bytes - $length);
            } else {
                $start = (int) $matches[1];
            }
            if ($matches[2] !== '') {
            ...
            if ($start > $end || $start >= $object->size_bytes) {
        ...
        }, $status, ['Content-Type' => $object->content_type ?: 'application/octet-stream', 'Content-Length' => (string) ($end - $start + 1), 'Content-Range' => $status === 206 ? 'bytes '.$start.'-'.$end.'/'.$object->size_bytes : null, 'ETag' => $etag, 'Accept-Ranges' => 'bytes']);
    }
    ...
    private function deleteObject(StorageBucket $bucket, string $key): Response
    {
        ...
        if ($object) {
            ...
            $bucket->decrement('usage_bytes', $object->size_bytes);
    ...
    private function deleteBucket(StorageBucket $bucket): Response
    {
        if ($bucket->objects()->exists()) {
            throw new S3Exception('BucketNotEmpty', 'The bucket you tried to delete is not empty.', 409, $bucket->name);
        }
        $bucket->delete();
    ...
    private function object(StorageBucket $bucket, string $key): StorageObject
    {
        ...
        if (! $object) {
            throw new S3Exception('NoSuchKey', 'The specified key does not exist.', 404, $bucket->name.'/'.$key);
        }
