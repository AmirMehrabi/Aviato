            ...
            @forelse($categories as $category)
                <form method="POST" action="{{ route('admin.ticket-categories.update', $category) }}" class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    ...
                    <div class="grid gap-4 lg:grid-cols-2">
                        ...
                        <label class="block"><span class="text-sm font-black text-slate-700">روش تخصیص</span><select name="assignment_strategy" class="mt-2 w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm font-bold">@foreach($strategies as $key => $label)<option value="{{ $key }}" @selected($category->assignment_strategy === $key)>{{ $label }}</option>@endforeach</select></label>
