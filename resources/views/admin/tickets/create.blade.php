    ...
    <form method="POST" action="{{ route('admin.tickets.store') }}" enctype="multipart/form-data" class="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div class="mt-5 grid gap-4 md:grid-cols-2">
                ...
                <label class="block"><span class="text-sm font-black text-slate-700">مسئول</span><select name="assigned_user_id" class="mt-2 w-full rounded-lg border border-slate-200 px-3 py-3 text-sm font-bold"><option value="">Auto assignment</option>@foreach($agents as $agent)<option value="{{ $agent->id }}" @selected(old('assigned_user_id') == $agent->id)>{{ $agent->name }}</option>@endforeach</select></label>
            </div>
