...
<head>
    ...
    <title>فراموشی رمز عبور مشتری | آویاتو</title>
        ...
        <div class="mx-auto flex min-h-[calc(100vh-3rem)] max-w-lg items-center">
            <section class="w-full overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl shadow-slate-950/10">
                <div class="border-b border-slate-200 bg-[linear-gradient(180deg,#EAF4FF_0%,#FFFFFF_100%)] px-6 py-7 md:px-8">
                    <span class="inline-flex rounded-md bg-white px-3 py-1 text-xs font-black text-[#0069FF] ring-1 ring-[#B8D6FF]">بازیابی رمز مشتری</span>
                    <h1 class="mt-4 text-3xl font-black leading-tight text-slate-950">دریافت کد OTP</h1>
                    <p class="mt-2 text-sm leading-7 text-slate-600">ایمیل یا موبایل حساب مشتری را وارد کنید. کد OTP از همان مسیر برای شما ارسال می شود.</p>
                </div>

                <form method="POST" action="{{ route('customer.password.send', [], false) }}" class="space-y-5 px-6 py-7 md:px-8" data-submit-loading>
                    @csrf
                    ...
                    <button class="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-[#0069FF] px-5 py-3.5 text-base font-black text-white shadow-lg shadow-[#0069FF]/20 transition hover:bg-[#0050D0]" type="submit">
                        <span>ارسال کد OTP</span>
                    </button>

                    <p class="text-center text-sm font-bold text-slate-500">
                        رمز عبور را به خاطر دارید؟
                        <a class="text-[#0069FF] transition hover:text-[#0050D0]" href="{{ route('customer.login', [], false) }}">ورود مشتری</a>
                    </p>
                </form>
            </section>
        </div>
