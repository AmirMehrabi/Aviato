...
<head>
    ...
    <title>رمز عبور جدید مشتری | آویاتو</title>
        ...
        <div class="mx-auto flex min-h-[calc(100vh-3rem)] max-w-lg items-center">
            <section class="w-full overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl shadow-slate-950/10">
                <div class="border-b border-slate-200 bg-[linear-gradient(180deg,#EAF4FF_0%,#FFFFFF_100%)] px-6 py-7 md:px-8">
                    <span class="inline-flex rounded-md bg-white px-3 py-1 text-xs font-black text-[#0069FF] ring-1 ring-[#B8D6FF]">رمز جدید</span>
                    <h1 class="mt-4 text-3xl font-black leading-tight text-slate-950">رمز عبور جدید را وارد کنید</h1>
                    <p class="mt-2 text-sm leading-7 text-slate-600">پس از ثبت رمز جدید، می توانید از صفحه ورود مشتری وارد شوید.</p>
                </div>

                <form method="POST" action="{{ route('customer.password.update', [], false) }}" class="space-y-5 px-6 py-7 md:px-8" data-submit-loading>
                    @csrf
                    ...
                    <label class="block">
                        <span class="text-sm font-black text-slate-700">رمز عبور جدید</span>
                    ...
                    <label class="block">
                        <span class="text-sm font-black text-slate-700">تکرار رمز عبور جدید</span>
...
@include('auth.partials.submit-loading', ['loadingText' => 'در حال ثبت...'])
</html>
