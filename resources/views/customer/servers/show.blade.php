...
@php
    ...
    $consoleReady = $server->isProxmox() && ! $server->isLxc() && $server->proxmoxServer && $server->node && $server->vmid && $server->provisioning_status === \App\Models\VirtualMachine::PROVISION_READY && ! $isLocked;
                ...
                <div class="relative grid gap-6 xl:grid-cols-[minmax(0,1fr)_420px] xl:items-end">
                    ...
                    <div class="rounded-3xl border border-white/10 bg-white p-5 text-slate-950 shadow-2xl shadow-slate-950/20">
                        ...
                        <div class="mt-4 flex flex-wrap gap-2">
                            ...
                            <a href="{{ route('customer.network.show', $server, false) }}" class="inline-flex flex-1 justify-center rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-black text-slate-700 transition hover:border-[#B8D6FF] hover:bg-[#EBF3FF] hover:text-[#0069FF]">مصرف شبکه</a>
