...
@section('content')
    <section class="relative isolate overflow-hidden bg-white px-4 pb-16 pt-16 md:px-8 md:pb-20 md:pt-24 lg:px-10">
        ...
        <div class="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[1fr_420px] lg:items-end">
            <div>
                <p class="text-sm  text-[#0069FF]">قیمت های روشن</p>
                <h1 class="mt-4 max-w-4xl text-4xl  leading-tight text-slate-950 md:text-6xl">پلن ماشین مجازی را راحت مقایسه کنید و بخرید.</h1>
            ...
            <div class="rounded-2xl border border-[#B8D6FF] bg-white p-6 shadow-xl shadow-[#0080FF]/10">
                <p class="text-sm  text-[#0069FF]">راهنمای سریع انتخاب</p>
                <div class="mt-5 space-y-4 text-sm leading-7 text-slate-600">
                    <p><span class=" text-slate-950">شروع:</span> مناسب سایت سبک، تست و پروژه های تازه.</p>
                    <p><span class=" text-slate-950">رشد:</span> انتخاب بهتر برای فروشگاه و اپلیکیشن های فعال.</p>
                    <p><span class=" text-slate-950">قدرت بیشتر:</span> مناسب دیتابیس، پردازش و ترافیک بالاتر.</p>
                </div>
            </div>
        </div>
    </section>
            ...
            <div class="grid gap-5 lg:grid-cols-3">
                @forelse ($pricingBundles as $bundle)
                    ...
                    <article class="relative rounded-2xl border p-6 shadow-sm {{ $loop->index === $featuredIndex ? 'border-[#0080FF] bg-[#EAF4FF] shadow-xl shadow-[#0080FF]/10' : 'border-slate-200 bg-white' }}">
                        ...
                        @endif
                        <p class="text-sm  text-[#0069FF]">{{ $copy['badge'] }}</p>
                        <h2 class="mt-4 text-3xl  text-slate-950">{{ $bundle->name }}</h2>
                        ...
                        <div class="mt-7 grid grid-cols-2 gap-2 text-center text-xs  text-slate-700">
                        ...
                        <a href="{{ route('customer.register') }}" class="mt-8 inline-flex w-full items-center justify-center rounded-lg bg-[#0069FF] px-5 py-3 text-sm  text-white transition hover:bg-[#0050D0]">خرید این پلن</a>
                    </article>
                @empty
                    <div class="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-10 text-center lg:col-span-3">
                        <h2 class="text-2xl  text-slate-950">فعلا پلن فعالی برای نمایش وجود ندارد.</h2>
        ...
        <section class="bg-[#06162E] px-4 py-20 text-white md:px-8 lg:px-10">
            <div class="mx-auto max-w-7xl">
                <div class="mb-10 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
                    <div>
                        <p class="text-sm  text-[#8FC7FF]">مقایسه منابع</p>
                        <h2 class="mt-3 text-3xl  leading-tight md:text-5xl">قبل از پرداخت، منابع و قیمت را ببینید</h2>
                    </div>
                ...
                <div class="overflow-hidden rounded-2xl border border-white/10 bg-white/[0.04]">
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-white/10 text-sm">
                            <thead class="bg-white/[0.03] text-right text-xs  text-slate-300">
                                <tr>
                            ...
                            <tbody class="divide-y divide-white/10">
                                @foreach ($pricingBundles as $bundle)
                                    <tr>
                                        <td class="whitespace-nowrap px-5 py-4  text-white">{{ $bundle->name }}</td>
                                        ...
                                        <td class="whitespace-nowrap px-5 py-4  text-white">{{ $wallets->format($bundle->monthly_price) }}</td>
    ...
    <section class="bg-[#EAF4FF] px-4 pb-20 pt-2 md:px-8 lg:px-10">
        <div class="mx-auto max-w-7xl overflow-hidden rounded-2xl bg-[#0069FF] p-8 text-white shadow-2xl shadow-[#0069FF]/20 md:p-12">
            <div class="grid gap-8 lg:grid-cols-[1fr_auto] lg:items-center">
                <div>
                    <h2 class="text-3xl  leading-tight md:text-5xl">پلن مناسب را پیدا کردید؟</h2>
                    <p class="mt-4 max-w-2xl leading-8 text-blue-50">حساب بسازید و سرور مجازی خود را با همین منابع سفارش دهید.</p>
                </div>
                <a href="{{ route('customer.register') }}" class="inline-flex items-center justify-center rounded-lg bg-white px-7 py-4 text-base  text-[#0069FF] shadow-xl transition hover:bg-blue-50">خرید سرور مجازی</a>
            </div>
        </div>
    </section>
@endsection
