...
@section('content')
    <section class="relative isolate overflow-hidden bg-[#06162E] px-4 pb-20 pt-16 text-white md:px-8 md:pb-24 md:pt-24 lg:px-10">
        ...
        <div class="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
            <div>
                <p class="text-sm  text-[#8FC7FF]">کاربردهای ماشین مجازی</p>
                <h1 class="mt-4 max-w-4xl text-4xl  leading-tight md:text-6xl">سرور مجازی را بر اساس نیاز خود انتخاب کنید.</h1>
                ...
                <div class="mt-9 flex flex-col gap-3 sm:flex-row">
                    <a href="{{ route('pricing') }}" class="inline-flex items-center justify-center rounded-lg bg-[#0080FF] px-7 py-4 text-base  text-white shadow-xl shadow-[#0080FF]/20 transition hover:bg-[#0069FF]">دیدن پلن ها</a>
                    <a href="{{ route('contact') }}" class="inline-flex items-center justify-center rounded-lg border border-white/20 px-7 py-4 text-base  text-white transition hover:bg-white/10">کمک برای انتخاب</a>
                </div>
            </div>
            <div class="rounded-2xl border border-white/10 bg-white/[0.05] p-6 shadow-2xl shadow-slate-950/20">
                <p class="text-sm  text-[#8FC7FF]">راهنمای سریع انتخاب</p>
    ...
    <section class="bg-white px-4 py-20 md:px-8 lg:px-10">
        <div class="mx-auto max-w-7xl">
            <div class="mb-10 max-w-3xl">
                <p class="text-sm  text-[#0069FF]">سناریوهای پیشنهادی</p>
                <h2 class="mt-3 text-3xl  leading-tight text-slate-950 md:text-5xl">برای هر نیاز، یک نقطه شروع ساده</h2>
            </div>
                ...
                @foreach ($solutions as $solution)
                    <article class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition hover:border-[#B8D6FF] hover:shadow-xl hover:shadow-slate-200/70">
                        <div class="grid gap-6 xl:grid-cols-[1fr_260px]">
                            <div>
                                <p class="text-sm  text-[#0069FF]">{{ $solution['best'] }}</p>
                                <h3 class="mt-3 text-2xl  text-slate-950">{{ $solution['name'] }}</h3>
                                <p class="mt-4 text-sm leading-8 text-slate-600">{{ $solution['summary'] }}</p>
                                <div class="mt-5 rounded-xl bg-[#F7FBFF] p-4">
                                    <p class="text-xs  text-slate-500">ابزارهای رایج</p>
                                    <p class="mt-2 text-sm font-bold leading-7 text-slate-700">{{ $solution['stack'] }}</p>
                                </div>
                            </div>
                            <aside class="rounded-xl bg-[#06162E] p-5 text-white">
                                <p class="text-xs  text-[#8FC7FF]">پلن پیشنهادی</p>
                                <h4 class="mt-2 text-2xl ">{{ $solution['bundle']?->name ?? 'بعد از انتشار پلن' }}</h4>
                                @if ($solution['bundle'])
                                    <p class="mt-3 text-xl ">{{ $wallets->format($solution['bundle']->monthly_price) }}</p>
                                    <p class="mt-1 text-xs text-slate-400">ماهانه</p>
                                    <div class="mt-5 grid grid-cols-2 gap-2 text-center text-xs ">
                        ...
                        <div class="mt-6 flex flex-col gap-3 sm:flex-row">
                            <a href="{{ route('customer.register') }}" class="inline-flex items-center justify-center rounded-lg bg-[#0069FF] px-5 py-3 text-sm  text-white transition hover:bg-[#0050D0]">شروع خرید</a>
                            <a href="{{ route('pricing') }}" class="inline-flex items-center justify-center rounded-lg border border-slate-200 px-5 py-3 text-sm  text-slate-700 transition hover:border-[#0080FF] hover:text-[#0069FF]">مقایسه پلن ها</a>
                        </div>
                    </article>
                @endforeach
            </div>
    ...
    <section class="bg-[#F7FBFF] px-4 py-20 md:px-8 lg:px-10">
        <div class="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:items-center">
            <div>
                <p class="text-sm  text-[#0069FF]">چرا این راهنما کمک می کند؟</p>
                <h2 class="mt-3 text-3xl  leading-tight text-slate-950 md:text-5xl">انتخاب سرور وقتی راحت تر است که کاربرد آن روشن باشد.</h2>
    ...
    <section class="bg-[#07172D] px-4 py-20 text-white md:px-8 lg:px-10">
        <div class="mx-auto max-w-7xl">
            <div class="grid gap-8 lg:grid-cols-[1fr_auto] lg:items-center">
                <div>
                    <p class="text-sm  text-[#8FC7FF]">نیاز خاص دارید؟</p>
                    <h2 class="mt-3 max-w-4xl text-3xl  leading-tight md:text-5xl">اگر مطمئن نیستید کدام پلن مناسب شماست، با ما صحبت کنید.</h2>
                ...
                <a href="{{ route('contact') }}" class="inline-flex items-center justify-center rounded-lg bg-white px-7 py-4 text-base  text-[#07172D] transition hover:bg-blue-50">تماس با ما</a>
            </div>
        </div>
    </section>
@endsection
